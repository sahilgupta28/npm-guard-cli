...
// api.npmjs.org rate-limits per IP; a big dependency tree checked back-to-back
// trips it even with retries, since every request is throttled, not just the
// unlucky ones. Serialize download-stat calls with a floor on their spacing
// so we never burst past the limit in the first place.
const DOWNLOADS_MIN_INTERVAL_MS = 300;
let downloadsQueue = Promise.resolve(0);

function sleep(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

function throttleDownloadsCall(run) {
  const scheduled = downloadsQueue.then(async (lastCallAt) => {
    const wait = lastCallAt + DOWNLOADS_MIN_INTERVAL_MS - Date.now();
    if (wait > 0) await sleep(wait);
    return Date.now();
  });
  downloadsQueue = scheduled.catch(() => Date.now());
