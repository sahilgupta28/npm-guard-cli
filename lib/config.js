...
function addToGlobalAllowlist(pkgName) {
  const config = getGlobalConfig();
  if (!config.allowlist.includes(pkgName)) config.allowlist.push(pkgName);
  writeJson(GLOBAL_CONFIG_PATH, config);
  return config;
}

function removeFromGlobalAllowlist(pkgName) {
  const config = getGlobalConfig();
  config.allowlist = config.allowlist.filter((p) => p !== pkgName);
  writeJson(GLOBAL_CONFIG_PATH, config);
  return config;
}
